...
X = bow_vectorizer.fit_transform(combined_data['text']).toarray()
...
print("Logistic Regression BoW 3000 Model Evaluation")
...
joblib.dump(bow_vectorizer, os.path.join(script_dir, 'Model\Lr\\lr_vectorizer_1000.pkl'))

# test the model
sample_text = ["Win a free iPhone now!", "Let's meet tomorrow."]
sample_processed = [preprocess_text(text) for text in sample_text]
sample_vector = bow_vectorizer.transform(sample_processed).toarray()
